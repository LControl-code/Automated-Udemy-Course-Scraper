...
export async function checkCourses (courses) {
  const transaction = startTransaction({
    op: 'checkCourses',
    name: 'Check if courses are still free'
  })
  ...
  try {
    ...
    const span1 = transaction.startChild({
      op: 'extractCourseInfo',
      description: 'Extract course IDs and coupon codes'
    })
    ...
    const span2 = transaction.startChild({
      op: 'checkCoursePrices',
      description:
        'Make HTTP request to check course prices and parse response'
    })
    ...
    const span3 = transaction.startChild({
      op: 'findFreeCourses',
      description: 'Find free courses in the response data'
    })
    ...
    if (data && data.courses) {
      freeCourses = Object.entries(data.courses)
        .filter(
          ([, course]) =>
            course.price.amount === 0 && course.price.price_string === 'Free'
        )
        .map(([id]) =>
          courses.find((course) => course.udemyCourseId === Number(id))
        )
    }
    ...
    const freeCourseIds = freeCourses.map((course) =>
      Number(course.udemyCourseId)
    )
    const filteredCourses = courses.filter((course) =>
      freeCourseIds.includes(Number(course.udemyCourseId))
    )
...
export async function checkEnrollment (course) {
  ...
  try {
    ...
    const span1 = transaction.startChild({
      op: 'checkCourseEnrollment',
      description:
        'Make HTTP request to check course enrollment and parse response'
    })
    const response = await fetch(
      `https://www.udemy.com/api-2.0/course-landing-components/${courseId}/me/?components=redeem_coupon`,
      {
        headers: {
          Cookie: cookieString
        }
    ...
    const span2 = transaction.startChild({
      op: 'checkPurchaseStatus',
      description: 'Check if course has already been purchased'
    })
