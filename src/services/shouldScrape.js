...
export default async function shouldScrape() {
  ...
  try {
    ...
    const span = transaction.startChild({ op: 'getETag', description: 'Getting ETag' });
    ...
    const span2 = transaction.startChild({ op: 'checkForUpdates', description: 'Checking for updates' });
    ...
    const span3 = transaction.startChild({ op: 'writeLog', description: 'Writing new ETag to log' });
    await writeLog(eTagFilePath, JSON.stringify({ eTag: newETag }, null, 2), 'r');
    span3.finish();
...
async function getETag(eTagFilePath) {
  ...
  try {
    const span = transaction.startChild({ op: 'fs.access', description: 'Accessing ETag file' });
  ...
  let eTagData = { eTag: '' };
  const span2 = transaction.startChild({ op: 'fs.readFile', description: 'Reading ETag file' });
...
async function checkForUpdates(url, oldETag) {
  ...
  try {
    const span = transaction.startChild({ op: 'axios.head', description: 'Sending HEAD request' });
